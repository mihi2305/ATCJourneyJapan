...
| Phase 2.0 | 完了 | 航空機データ拡張 | 便名 / 機種 / 運航種別 / 使用滑走路 / ゲート/スポット / 状態 / 担当管制ポジション / 出発地/目的地 / 推奨コマンドID / A滑走路をRWY_A・18L/36Rとして扱う土台 / B滑走路は将来拡張用データのみ |
| Phase 2.2 | 未着手 | 複数機運用 | 到着2機 / 出発2機 / 時間差出現 / マルチタスク発生 |
| Phase 2.3 | 次に実装予定 | フライトストリップ方式 | 左側に到着 ARRIVAL / 出発 DEPARTURE リスト / 便名帯で航空機を選択 / 未指示機をハイライト / 機体クリック依存から脱却 |
